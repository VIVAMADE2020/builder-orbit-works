...
const ConsolidamentoDebiti: React.FC = () => {
  ...
  const calculateTotals = () => {
    const totalAmount = currentDebts.reduce(
      (sum, debt) => sum + debt.amount,
      0,
    );
    const totalPayment = currentDebts.reduce(
      (sum, debt) => sum + debt.payment,
      0,
    );
    const weightedRate =
      currentDebts.reduce((sum, debt) => sum + debt.rate * debt.amount, 0) /
      totalAmount;

    // Calcolo con consolidamento al 2%
    const consolidatedPayment =
      (((totalAmount * 0.02) / 12) * Math.pow(1 + 0.02 / 12, 60)) /
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-xl text-gray-600 mb-8 leading-relaxed">
                Semplifica la tua vita finanziaria! Unifica tutti i tuoi debiti
                in un'unica rata conveniente con il nostro servizio di
                consolidamento debiti. Risparmia sugli interessi e riprendi il
                controllo.
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {debtTypes.map((debt, index) => (
              <div key={index} className="bg-white p-8 rounded-xl shadow-lg">
                <div
                  className={`w-16 h-16 ${debt.color} rounded-full flex items-center justify-center text-white mb-6`}
                >
                  {debt.icon}
                </div>
                <h3 className="text-xl font-bold text-primary mb-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-primary/10 p-4 rounded-lg border-2 border-primary/30">
                    ...
                    <div className="text-sm text-gray-600">
                      Debito totale: €{totals.totalAmount.toLocaleString()} -
                      Tasso medio: {totals.weightedRate.toFixed(1)}%
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-lg text-gray-600 mb-8">
                Verifica se puoi accedere al nostro servizio di consolidamento
                debiti
              </p>
              ...
              <div className="mt-8 p-6 bg-secondary/10 rounded-xl border border-secondary/20">
                ...
                <p className="text-gray-700">
                  I nostri esperti valutano gratuitamente la tua situazione e ti
                  consigliano la soluzione migliore per il consolidamento dei
                  tuoi debiti.
                </p>
              </div>
            </div>
      ...
      <section className="py-20 bg-secondary">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-200 mb-8 max-w-2xl mx-auto">
            Basta con rate multiple e tassi elevati. Consolida tutti i tuoi
            debiti in un'unica soluzione conveniente e riprendi il controllo
            delle tue finanze.
          </p>
